...
const getMeasureTotalFineByManzanaLote = async () => {
  try {
    const measure = await dbConnection.query(
      `SELECT m.idCliente, m.nombre, m.codigo, m.manzana, M.lote, COUNT(DISTINCT d.idMultaDetalle) AS cantidadMultas FROM JA_Medida m LEFT JOIN JA_MultaDetalle d ON m.idCliente = d.id_cliente WHERE  d.pagado = 0 GROUP BY m.idCliente, m.Manzana, m.Lote,m.Nombre,m.codigo order by m.manzana, m.lote`,
      {
        type: sequelize.QueryTypes.SELECT,
      }
    );
